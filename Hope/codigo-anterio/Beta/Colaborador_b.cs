...
namespace Hope.Beta
{
    class Colaborador_b : Colaborador_abs
    {
        ...
        protected override bool novo_insert_registro()
        {
            try
            {
                HopeDataSet.ColadoradorRow row = Hope_c.localSet.Coladorador.NewColadoradorRow();
                ...
                Hope_c.localSet.Coladorador.Rows.Add(row);
                Hope_c.localSet.AcceptChanges();
                Hope_c.informacao.Add("000", "Regstro gravado");
                return true;
            }
            catch (Exception e)
            {
        ...
        {
            if (Hope_c.localSet.Coladorador.Count != 0)
            {
                foreach (HopeDataSet.ColadoradorRow item in Hope_c.localSet.Coladorador.Rows)
                {
                    if (item.Login_User.Equals(_Logado._Login_User))
                    {
                        if (item.Login_Pass.Equals(_Logado._Login_PassWord))
                        {
                            ...
                            Hope_c.informacao.Add("000", "Login executato sucesso");
                            Autenticado = true;
                            return true;
                        }
                    }
                }
                Hope_c.informacao.Add("000", "Login executato falha nao enontrado registro");
                return false;
            }
            else
            {
                Hope_c.informacao.Add("000", "DataRow Usuario Count = 0");
                return false;
            }
        }
        ...
        protected override bool select_all_from_registro(out Entidade.IColaborador_Ent_c[] ListResultado)
        {
            ...
            if (Hope_c.localSet.Coladorador.Count != 0)
            {
                ...
                for (int i = 0; i < Hope_c.localSet.Coladorador.Count; i++)
                {
                   list.Add(new Beta.Colaborador_Ent_b() {
                        _ID_Colaborador = (int)Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.ID_ColaboradorColumn],
                        _Login_User = Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.Login_UserColumn].ToString(),
                        _Login_PassWord = Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.Login_PassColumn].ToString(),
                        _Nome_Vendedor = Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.Nome_VendedorColumn].ToString()
                    });
                }
        ...
        protected override bool focus_update_registro()
        {
            if (Hope_c.localSet.Coladorador.Count != 0)
            {
                for (int i = 0; i < Hope_c.localSet.Coladorador.Count; i++)
                {
                    if ((int)Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.ID_ColaboradorColumn] == _FocusRegistro._ID_Colaborador)
                    {
                        Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.Login_PassColumn] = _AlteraRegistro._Login_PassWord;
                        Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.Login_UserColumn] = _AlteraRegistro._Login_User;
                        Hope_c.localSet.Coladorador.Rows[i][Hope_c.localSet.Coladorador.Nome_VendedorColumn] = _AlteraRegistro._Nome_Vendedor;
                        Hope_c.localSet.AcceptChanges();
                        Hope_c.informacao.Add("000", "DataRow Usuario modificado");
                        return true;
                    }
                }
            }
