...
namespace Hope.Beta
{
    class Comprador_b : Comprador_abs
    {
        ...
        protected override bool focus_update_registro()
        {
            try
            {
                for (int i = 0; i < Hope_c.localSet.Comprador.Count; i++)
                {
                    HopeDataSet.CompradorRow comprador = Hope_c.localSet.Comprador.Rows[i] as HopeDataSet.CompradorRow;
        ...
        {
            HopeDataSet.CompradorRow comprador = Hope_c.localSet.Comprador.NewCompradorRow();
        ...
        protected override bool select_all_from(out Entidade.IComprador_Ent_c[] listaressultado)
        {
            if (Hope_c.localSet.Comprador.Count != 0)
            {
                ...
                for (int i = 0; i < Hope_c.localSet.Comprador.Count; i++)
                {
                    ...
                    {
                        _Id_Comprador = (int)Hope_c.localSet.Comprador.Rows[i]
                        [Hope_c.localSet.Comprador.ID_CompradorColumn],
                        _Numero_Registro = Hope_c.localSet.Comprador.Rows[i]
                        [Hope_c.localSet.Comprador.Numero_RegistroColumn].ToString(),
                        _NomeCompleto_RazaoSocial = Hope_c.localSet.Comprador.Rows[i]
                        [Hope_c.localSet.Comprador.NomeCompleto_RazaoSocialColumn].ToString()
                    });
                }
